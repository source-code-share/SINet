...
def count_flops_test(net, device, dtype, input_size,batch_size=1):
    net = add_flops_counting_methods(net)

    net.to(device=device, dtype=dtype)
    net = net.train()

    batch = torch.randn(batch_size, 3, input_size, input_size).to(device=device, dtype=dtype)
    net.start_flops_count()

    _ = net(batch)
    param=net.compute_average_flops_cost() / 2  # Result in FLOPs
    print('FLOPs: {:.2f}M'.format(param/1000000))

def time_count(net, dtype, input_size):
    import time
    device='cpu'
    net = add_flops_counting_methods(net)
    ...
    batch = torch.randn(4, 3, input_size, input_size).to(device=device, dtype=dtype)
    net.start_flops_count()

    start=time.time()
    _ = net(batch)
    dur=time.time()-start
    print('TIMES: {:.2f}ms'.format(dur*60))