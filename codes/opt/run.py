...
from opt.clr import CyclicLR


def train(model, loader, epoch, optimizer, criterion, device, dtype, batch_size, log_interval,writer):
    model.train()
    ...
    for batch_idx, (data, target) in enumerate(loader):
        ...
        writer.add_scalar('/train_loss', loss.item(), epoch*batch_size+(batch_idx + 1))
        writer.add_scalar('/train_top1', 100. * correct1 / (batch_size * (batch_idx + 1)),
                          epoch*batch_size+(batch_idx + 1))
        writer.add_scalar('/train_top5', 100. * correct5 / (batch_size * (batch_idx + 1)),
                          epoch*batch_size+(batch_idx + 1))
